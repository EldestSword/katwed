-- Expand the controlled visual-theme catalogue to Batch 3 without changing quiz data,
...
as $$
  select exists (
    ...
    from (values
      ...
      ('holographic', 'holographic-prism'),
      ('holographic', 'holographic-foil'),
      ('holographic', 'holographic-orbit'),
      ('glass', 'glass-frost'),
      ('glass', 'glass-refraction'),
      ('glass', 'glass-panels'),
      ('comic-book', 'comic-book-halftone'),
      ('comic-book', 'comic-book-panels'),
      ('comic-book', 'comic-book-impact'),
      ('vhs', 'vhs-tracking'),
      ('vhs', 'vhs-chroma'),
      ('vhs', 'vhs-static'),
      ...
      ('laboratory', 'laboratory-molecules'),
      ('laboratory', 'laboratory-liquid'),
      ('laboratory', 'laboratory-spectrum'),
      ('jungle', 'jungle-canopy'),
      ('jungle', 'jungle-depth'),
      ('jungle', 'jungle-tropical'),
      ('deep-ocean', 'deep-ocean-abyss'),
      ('deep-ocean', 'deep-ocean-current'),
      ('deep-ocean', 'deep-ocean-biolume'),
      ('circus', 'circus-big-top'),
      ('circus', 'circus-lights'),
      ('circus', 'circus-marquee'),
      ('sports-broadcast', 'sports-broadcast-motion'),
      ('sports-broadcast', 'sports-broadcast-studio'),
      ('sports-broadcast', 'sports-broadcast-arena'),
      ('awards-show', 'awards-show-spotlight'),
      ('awards-show', 'awards-show-curtain'),
      ('awards-show', 'awards-show-sparkle'),
      ('new-year', 'new-year-midnight'),
      ('new-year', 'new-year-firework'),
      ('new-year', 'new-year-confetti'),
      ('autumn', 'autumn-embers'),
      ('autumn', 'autumn-leaves'),
      ('autumn', 'autumn-harvest'),
      ('winter', 'winter-frost'),
      ('winter', 'winter-snowlight'),
      ('winter', 'winter-ice'),
...
alter table public.quizzes
  ...
  add constraint quizzes_theme_id_check check (theme_id in (
    ...
    'holographic', 'glass', 'comic-book', 'vhs', 'newspaper', 'laboratory',
    'jungle', 'deep-ocean', 'circus', 'sports-broadcast', 'awards-show',
    'new-year', 'autumn', 'winter', 'summer'
  )),
...
-- Preserve the stale-client behaviour established by Batches 1 and 2 while extending
-- the authoritative compatibility check to every Batch 3 pair.
alter function public.host_save_quiz(jsonb)
  rename to host_save_quiz_without_visual_theme_batch_3;

revoke all on function public.host_save_quiz_without_visual_theme_batch_3(jsonb)
...
begin
  ...
  return public.host_save_quiz_without_visual_theme_batch_3(v_forward_quiz);
end;
$$;
