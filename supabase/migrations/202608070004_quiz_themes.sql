-- Persist one of the six curated audience themes per quiz and expose it through
-- owner quiz reads and the player-safe game-state boundary.

alter table public.quizzes
  add column theme_id text not null default 'katwed';

alter table public.quizzes
  add constraint quizzes_theme_id_check
  check (theme_id in ('katwed', 'midnight', 'sunset', 'arcade', 'mint', 'paper'));

create or replace function public.quiz_to_json(p_quiz_id uuid)
returns jsonb language sql stable security definer set search_path = public as $$
  select jsonb_build_object(
    'id', q.id, 'title', q.title, 'coverImagePath', q.cover_image_path, 'themeId', q.theme_id,
...
begin
  ...
  if nullif(p_quiz ->> 'id', '') is null then
    insert into public.quizzes (owner_id, title, cover_image_path, theme_id)
    values (
      auth.uid(),
      trim(p_quiz ->> 'title'),
