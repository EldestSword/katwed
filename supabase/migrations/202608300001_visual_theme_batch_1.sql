-- Expand the controlled visual-theme catalogue to Batch 1 without changing quiz data,
...
-- The deployed save chain deliberately preserves a background when an older client
-- sends a compatible theme without a backgroundId field. Supply the authoritative
-- current value to that chain so the same behaviour covers every Batch 1 pair.
alter function public.host_save_quiz(jsonb)
  rename to host_save_quiz_without_visual_theme_batch_1;

revoke all on function public.host_save_quiz_without_visual_theme_batch_1(jsonb)
...
begin
  ...
  return public.host_save_quiz_without_visual_theme_batch_1(v_forward_quiz);
end;
$$;
