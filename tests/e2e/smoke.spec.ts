...
test('Head-to-Head authoring and a true two-player untimed game work end to end', async ({ context, page }) => {
  ...
  let settings = await openQuizSettings(page, 'Game')
  ...
  await expect(page.getByText('Quiz saved.')).toBeVisible()
  await page.reload()
  settings = await openQuizSettings(page, 'Game')
...
test('the imported theme catalogue persists a Batch 3 theme through duplication and audience game phases', async ({ context, page }) => {
  ...
  const desktopEditorViewport = page.viewportSize()
  const promptInput = page.getByLabel('Prompt').first()
  const originalPrompt = await promptInput.inputValue()
  await page.setViewportSize({ width: 390, height: 844 })
  await promptInput.fill('A deliberately long question that verifies the mobile editor navigation keeps every question title inside its available width without creating a horizontal scrollbar')
  const mobileQuestionListGeometry = await page.locator('.question-navigator ol').evaluate((element) => {
    const list = element as unknown as BrowserEvaluationElement
    return { clientWidth: list.clientWidth, scrollWidth: list.scrollWidth }
  })
  ...
  await expect(themePicker.locator('.quiz-theme-option')).toHaveCount(5)
  await expect(themePicker.getByRole('button', { name: /Arcade/ })).toBeVisible()
  await expect(themePicker.getByRole('button', { name: /Retro Game Show/ })).toBeVisible()
  await expect(themePicker.getByRole('button', { name: /Sports Broadcast/ })).toBeVisible()
  await themePicker.getByRole('button', { name: 'Wildcards', exact: true }).click()
  await expect(themePicker.locator('.quiz-theme-option')).toHaveCount(6)
  await expect(themePicker.getByRole('button', { name: /Deep Ocean/ })).toBeVisible()
  await themePicker.getByRole('button', { name: 'Places & Culture', exact: true }).click()
  await expect(themePicker.locator('.quiz-theme-option')).toHaveCount(3)
  await expect(themePicker.getByRole('button', { name: /Greek/ })).toBeVisible()
  await expect(themePicker.getByRole('button', { name: /French/ })).toBeVisible()
  await expect(themePicker.getByRole('button', { name: /Italian/ })).toBeVisible()
  ...
  await selectQuizSettingsSection(settings, 'Backgrounds')
  backgroundPicker = settings.getByRole('group', { name: 'Quiz background' })
  await backgroundPicker.getByRole('button', { name: /Pixels/ }).click()
  const chiptunePreview = page.getByLabel('Chiptune theme preview')
  await expect(chiptunePreview.getByRole('heading')).toHaveCSS('font-family', /Pixelify Sans/)
  ...
  await selectQuizSettingsSection(settings, 'Backgrounds')
  backgroundPicker = settings.getByRole('group', { name: 'Quiz background' })
  await expect(backgroundPicker.getByRole('button', { name: /Frost/ })).toBeVisible()
  await expect(backgroundPicker.getByRole('button', { name: /Refraction/ })).toBeVisible()
  await expect(backgroundPicker.getByRole('button', { name: /Panels/ })).toBeVisible()
  await backgroundPicker.getByRole('button', { name: /Frost/ }).click()
  const preview = page.getByLabel('Glass theme preview')
  await expect(preview).toHaveAttribute('data-quiz-theme', 'glass')
  await expect(preview).toHaveCSS('font-family', /system-ui/)
  await expect(preview.getByRole('heading')).toHaveCSS('font-family', /Space Grotesk/)
  await expect(preview).toHaveAttribute('data-quiz-background', 'glass-frost')
  ...
  await expect(page.locator('.presentation-stage')).toHaveAttribute('data-quiz-theme', 'glass')
  await expect(page.locator('.presentation-stage')).toHaveAttribute('data-quiz-background', 'glass-frost')

  const presentation = await context.newPage()
  await presentation.goto(page.url().replace('/control', '/present'))
  await expect(presentation.locator('.presentation-stage')).toHaveAttribute('data-quiz-theme', 'glass')
  await expect(presentation.locator('.presentation-stage')).toHaveAttribute('data-quiz-background', 'glass-frost')
  const player = await joinPlayer(context, roomCode, 'Theme Player')
  await expect(player.locator('.player-game')).toHaveAttribute('data-quiz-theme', 'glass')
  await expect(player.locator('.player-game')).toHaveAttribute('data-quiz-background', 'glass-frost')
...
test('quiz covers persist across the library lifecycle and remain independent after duplication', async ({ page }) => {
  ...
  let settings = await openQuizSettings(page, 'Cover')
  ...
  settings = await openQuizSettings(page, 'Cover')
...
test('a three-player mixed-format quiz night survives refresh, intros and a Typed Answer override', async ({ context, page }) => {
  test.setTimeout(180_000)
  await enterHost(page)
  await page.getByRole('article', { name: 'Katwed! Mixed Quiz' }).getByRole('link', { name: 'Edit' }).click()
  await page.locator('.question-navigator').getByRole('button').filter({ hasText: 'Name the science-fiction programme' }).click()
  await page.getByText('Scoring', { exact: true }).click()
  await page.getByRole('checkbox', { name: 'Double score', exact: true }).check()
  await page.getByRole('checkbox', { name: 'Faster answers score more' }).check()
  await page.getByRole('button', { name: 'Save quiz' }).first().click()
  await expect(page.getByText('Quiz saved.')).toBeVisible()
  await page.locator('.editor-toolbar').getByRole('link', { name: '← Quizzes' }).click()
  ...
  const roomCode = await launchQuiz(page, 'Katwed! Mixed Quiz', async (setup) => {
    ...
    await setup.getByRole('checkbox', { name: 'Shuffle all answer choices' }).check()
    await setup.getByRole('checkbox', { name: /Auto-close answers/ }).uncheck()
    await expect(setup.getByRole('checkbox', { name: /Show live player answers/ })).toBeChecked()
    for (const viewport of [{ width: 1366, height: 768 }, { width: 1440, height: 900 }]) {
      await setup.setViewportSize(viewport)
      await expect(setup.getByRole('button', { name: 'Start lobby' })).toBeVisible()
      expect(await setup.evaluate<boolean>('document.documentElement.scrollWidth <= document.documentElement.clientWidth')).toBe(true)
    }
  })
  await page.evaluate((code) => {
    const storageKey = 'katwed.demo.state.v2'
    const raw = localStorage.getItem(storageKey)
    if (!raw) throw new Error('Demo session state was not persisted')
    const state = JSON.parse(raw) as { sessions: Array<{ roomCode: string; settings: { doubleScoreIntroMs: number } }> }
    const launched = state.sessions.find((candidate) => candidate.roomCode === code)
    if (!launched) throw new Error('Launched demo session was not found')
    launched.settings.doubleScoreIntroMs = 9000
    localStorage.setItem(storageKey, JSON.stringify(state))
  }, roomCode)
  await page.reload()
  const presentation = await context.newPage()
  ...
  await expect(zeroScorePlayer.getByRole('heading', { name: /You’re in, A Long Zero Score Player/ })).toBeVisible()
  ...
  for (const viewport of [{ width: 1366, height: 768 }, { width: 1440, height: 900 }, { width: 1920, height: 1080 }]) {
    await page.setViewportSize(viewport)
    const controls = await page.getByRole('group', { name: 'Game controls' }).boundingBox()
    if (!controls) throw new Error('Controller actions were not visible')
    expect(controls.y).toBeGreaterThanOrEqual(0)
    expect(controls.y + controls.height).toBeLessThanOrEqual(viewport.height)
  }

  let lockedRecoveryChecked = false
  async function revealRound(expectedReveal: RegExp) {
    ...
    await expect(page.getByRole('button', { name: 'Reveal answer' })).toBeVisible()
    await expect(presentation.getByRole('heading', { name: 'Answers locked' })).toBeVisible()
    if (!lockedRecoveryChecked) {
      await Promise.all([page.reload(), presentation.reload(), zeroScorePlayer.reload()])
      await expect(page.getByRole('button', { name: 'Reveal answer' })).toBeVisible()
      await expect(presentation.getByRole('heading', { name: 'Answers locked' })).toBeVisible()
      await expect(zeroScorePlayer.getByRole('heading', { name: 'Answer locked' })).toBeVisible()
      lockedRecoveryChecked = true
    }
    await page.getByRole('button', { name: 'Reveal answer' }).click()
  ...
  for (const viewport of [{ width: 390, height: 844 }, { width: 768, height: 1024 }, { width: 1440, height: 900 }]) {
    ...
    await expect(playerOne.locator('.player-reveal-outcome')).toBeVisible()
    await expect(playerOne.getByRole('group', { name: 'Correct answer' })).toBeVisible()
    expect(await playerOne.evaluate<boolean>('document.documentElement.scrollWidth <= document.documentElement.clientWidth')).toBe(true)
  }
  await playerOne.setViewportSize({ width: 320, height: 740 })
  ...
  await expect(presentation.getByRole('heading', { name: 'DOUBLE SCORE!' })).toBeVisible()
  await expect(zeroScorePlayer.getByRole('heading', { name: 'DOUBLE SCORE!' })).toBeVisible()
  await presentation.waitForTimeout(500)
  await Promise.all([presentation.reload(), zeroScorePlayer.reload()])
  await expect(presentation.getByRole('heading', { name: 'DOUBLE SCORE!' })).toBeVisible()
  await expect(zeroScorePlayer.getByRole('heading', { name: 'DOUBLE SCORE!' })).toBeVisible()
  await expect(presentation.getByText('Name the science-fiction programme')).toHaveCount(0)
  await page.reload()
  ...
  await playerTwo.getByRole('button', { name: 'Lock in' }).click()
  await expect(page.getByText('Waiting for: A Long Zero Score Player')).toBeVisible()
  const rileyResponse = page.locator('.controller-response-row').filter({ hasText: 'Riley' })
  await expect(rileyResponse.getByText('Star Trek')).toBeVisible()
  await page.getByRole('button', { name: 'Close answers now' }).click()
  await rileyResponse.getByRole('button', { name: 'Mark correct' }).click()
  await expect(rileyResponse.getByText('Host accepted ✓')).toBeVisible()
  await page.getByRole('button', { name: 'Reveal answer' }).click()
  await expect(presentation.locator('.presentation-reveal')).toContainText(/Red Dwarf/i)
  await expect(playerTwo.getByRole('heading', { name: 'You got it right!' })).toBeVisible()
  await expect(rileyResponse.getByText('Correct', { exact: true })).toBeVisible()
  await rileyResponse.getByRole('button', { name: 'Undo override' }).click()
  await expect(playerTwo.getByRole('heading', { name: 'Not this time' })).toBeVisible()
  await rileyResponse.getByRole('button', { name: 'Mark correct' }).click()
  await expect(playerTwo.getByRole('heading', { name: 'You got it right!' })).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Show leaderboard' }).click()
  const rileyStanding = presentation.locator('.leaderboard--presentation li').filter({ hasText: 'Riley' })
  await expect(rileyStanding).toContainText(/\d[\d,]+ points/)
  const rileyPoints = Number((await rileyStanding.textContent())?.match(/([\d,]+) points/)?.[1].replaceAll(',', ''))
  expect(rileyPoints).toBeGreaterThan(0)
  await page.getByRole('button', { name: 'Next question' }).click()
  ...
  const quinnFinal = presentation.locator('.final-podium li').filter({ hasText: 'Quinn' })
  const quinnPoints = Number((await quinnFinal.textContent())?.match(/([\d,]+) points/)?.[1].replaceAll(',', ''))
  expect(quinnPoints).toBeGreaterThan(7001)
  await page.reload()
  await presentation.reload()
  await zeroScorePlayer.reload()
