...
test('the imported theme catalogue persists a Batch 2 theme through duplication and audience game phases', async ({ context, page }) => {
  test.setTimeout(120_000)
  await enterHost(page)
  ...
  await expect(themePicker.locator('.quiz-theme-option')).toHaveCount(36)
  ...
  await expect(themePicker.locator('.quiz-theme-option')).toHaveCount(2)
  ...
  await themePicker.getByRole('button', { name: /Greek/ }).click()
  backgroundPicker = settings.getByRole('group', { name: 'Quiz background' })
  await expect(backgroundPicker.getByRole('button', { name: /Aegean/ })).toBeVisible()
  await expect(backgroundPicker.getByRole('button', { name: /Mosaic/ })).toBeVisible()
  await expect(backgroundPicker.getByRole('button', { name: /Sun/ })).toBeVisible()
  await backgroundPicker.getByRole('button', { name: /Mosaic/ }).click()
  const preview = page.getByLabel('Greek theme preview')
  await expect(preview).toHaveAttribute('data-quiz-theme', 'greek')
  await expect(preview).toHaveCSS('font-family', /Bricolage Grotesque/)
  await expect(preview.getByRole('heading')).toHaveCSS('font-family', /Cinzel/)
  await expect(preview).toHaveAttribute('data-quiz-background', 'greek-mosaic')
  await expect(preview).toHaveCSS('background-image', /greek-mosaic\.webp/)
  ...
  await expect(settings.getByRole('group', { name: 'Quiz theme' }).getByRole('button', { name: /Greek/ }))
    .toHaveAttribute('aria-pressed', 'true')
  await expect(settings.getByRole('group', { name: 'Quiz background' }).getByRole('button', { name: /Mosaic/ }))
    .toHaveAttribute('aria-pressed', 'true')
  await settings.getByRole('button', { name: 'Done' }).click()
  ...
  await expect(page.locator('.presentation-stage')).toHaveAttribute('data-quiz-theme', 'greek')
  await expect(page.locator('.presentation-stage')).toHaveAttribute('data-quiz-background', 'greek-mosaic')

  const presentation = await context.newPage()
  await presentation.goto(page.url().replace('/control', '/present'))
  await expect(presentation.locator('.presentation-stage')).toHaveAttribute('data-quiz-theme', 'greek')
  await expect(presentation.locator('.presentation-stage')).toHaveAttribute('data-quiz-background', 'greek-mosaic')
  const player = await joinPlayer(context, roomCode, 'Theme Player')
  await expect(player.locator('.player-game')).toHaveAttribute('data-quiz-theme', 'greek')
  await expect(player.locator('.player-game')).toHaveAttribute('data-quiz-background', 'greek-mosaic')
