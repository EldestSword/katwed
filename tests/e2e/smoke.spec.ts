...
test('a Batch 1 quiz theme persists through duplication and audience game phases', async ({ context, page }) => {
  test.setTimeout(120_000)
  await enterHost(page)
  ...
  await expect(themePicker.locator('.quiz-theme-option')).toHaveCount(21)
  ...
  await themePicker.getByRole('button', { name: /Hard Rock/ }).click()
  backgroundPicker = settings.getByRole('group', { name: 'Quiz background' })
  await expect(backgroundPicker.getByRole('button', { name: /Stage Lights/ })).toBeVisible()
  await expect(backgroundPicker.getByRole('button', { name: /Amps/ })).toBeVisible()
  await expect(backgroundPicker.getByRole('button', { name: /Electric Storm/ })).toBeVisible()
  await backgroundPicker.getByRole('button', { name: /Stage Lights/ }).click()
  const preview = page.getByLabel('Hard Rock theme preview')
  await expect(preview).toHaveAttribute('data-quiz-theme', 'hard-rock')
  await expect(preview).toHaveCSS('font-family', /Bricolage Grotesque/)
  await expect(preview.getByRole('heading')).toHaveCSS('font-family', /Oswald/)
  await expect(preview).toHaveAttribute('data-quiz-background', 'hard-rock-stage-lights')
  await expect(preview).toHaveCSS('background-image', /hard-rock-stage-lights\.webp/)
  ...
  await expect(settings.getByRole('group', { name: 'Quiz theme' }).getByRole('button', { name: /Hard Rock/ }))
    .toHaveAttribute('aria-pressed', 'true')
  await expect(settings.getByRole('group', { name: 'Quiz background' }).getByRole('button', { name: /Stage Lights/ }))
    .toHaveAttribute('aria-pressed', 'true')
  await settings.getByRole('button', { name: 'Done' }).click()
  ...
  await expect(page.locator('.presentation-stage')).toHaveAttribute('data-quiz-theme', 'hard-rock')
  await expect(page.locator('.presentation-stage')).toHaveAttribute('data-quiz-background', 'hard-rock-stage-lights')

  const presentation = await context.newPage()
  await presentation.goto(page.url().replace('/control', '/present'))
  await expect(presentation.locator('.presentation-stage')).toHaveAttribute('data-quiz-theme', 'hard-rock')
  await expect(presentation.locator('.presentation-stage')).toHaveAttribute('data-quiz-background', 'hard-rock-stage-lights')
  const player = await joinPlayer(context, roomCode, 'Theme Player')
  await expect(player.locator('.player-game')).toHaveAttribute('data-quiz-theme', 'hard-rock')
  await expect(player.locator('.player-game')).toHaveAttribute('data-quiz-background', 'hard-rock-stage-lights')
