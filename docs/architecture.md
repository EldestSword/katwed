...
The live Supabase project has applied every migration through `202608070005_quiz_backgrounds.sql`. Applied files are immutable history; production changes require a new chronological forward migration. `202608070006_head_to_head_foundation.sql` is pending and remains mutable only until a deliberate production application.
...
`202608070006_head_to_head_foundation.sql` is the sole pending migration. It adds a backward-compatible `standard` quiz-type default, the owner-scoped two-row competitor model and same-quiz nullable question assignment. Owner serialisation and save gain the new authored fields, while missing update keys preserve existing Head-to-Head configuration for stale clients. The migration recreates the host launch RPC only to reject Head-to-Head definitions before any room is resumed or created. It does not change player-safe game state, scoring, phases, joining, reconnect or answer submission.
