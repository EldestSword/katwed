...
The production schema and matching Netlify frontend are deployed through `202608070007_head_to_head_live_play.sql`. The single pending migration `202608080001_typed_answer.sql` adds Typed Answer persistence and authoritative scoring; it is unapplied and the matching frontend is not deployed. Active and archived libraries are separate repository queries. Archive and permanent deletion reject quizzes with active rooms, archived quizzes cannot launch, and permanent deletion requires an archived quiz. A database trigger preserves those rules even if an authenticated client attempts a direct row operation.
...
The existing 24-tile image effect derives a seeded Fisher–Yates order from the media path and authoritative question-open timestamp. The pure helper yields the same reveal rank for controller, presentation and player DOMs across rerenders and reconnects, while a different opening yields a different order. Rendering uses no `Math.random()`, and reduced-motion continues to show the complete image immediately.
...
The live Supabase project has applied every migration through `202608070007_head_to_head_live_play.sql`. Applied files are immutable history; production changes require a new chronological forward migration. `202608080001_typed_answer.sql` is the single pending migration. A deliberate release must apply it before deploying the matching Typed Answer/frontend-format-v2 bundle.
...
`202608080001_typed_answer.sql` is pending and unapplied. It replaces only the question discriminator constraint and the owner-save, answer-submit and safe-state definitions needed for Typed Answer, preserving the `007` live-play and security boundaries. No Supabase or Netlify release was performed for Typed Answer, format v2 or deterministic tile order.
