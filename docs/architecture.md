...
The dashboard retains the parsed definition only for an explicit confirmation step and renders a separate spoiler-safe metadata summary; answer-bearing question data is never placed in the preview DOM. Export is read-only and available for Active and Archived quizzes. Version 1 preserves cover, question and option media references without uploading or embedding binary data, so same-deployment sharing retains the existing exact-reference deletion protection but cross-environment media portability is not promised. This feature changes no repository contract, RPC or database migration.

Per-quiz themes use a central six-entry typed registry (`katwed`, `midnight`, `sunset`, `arcade`, `mint`, `paper`) and scoped CSS custom properties rather than separate components or a styling dependency. A second typed registry owns the exact 18 built-in backgrounds: three static repository assets per theme. `backgroundId = null` is Theme default and adds no image. Read boundaries defensively discard unknown or wrong-theme IDs, save validation rejects them, and the database has the same explicit theme/background compatibility rule. Changing theme clears an incompatible background without choosing or remembering a replacement.
...
The live Supabase project has applied every migration through `202608070005_quiz_backgrounds.sql`. Applied files are immutable history; production changes require a new chronological forward migration. `202608070006_head_to_head_foundation.sql` is pending but is treated as immutable by Stage 2 development. `202608070007_head_to_head_live_play.sql` is the only new migration and is also pending. A deliberate release must apply `006` then `007` before deploying the matching frontend.
...
`202608070006_head_to_head_foundation.sql` is the first pending migration. It adds a backward-compatible `standard` quiz-type default, the owner-scoped two-row competitor model and same-quiz nullable question assignment. Owner serialisation and save gain the authored fields, while missing update keys preserve existing Head-to-Head configuration for stale clients. Its launch guard remains byte-for-byte unchanged.

`202608070007_head_to_head_live_play.sql` is the second pending migration. It adds nullable competitor identity to players with a per-session unique claim, explicit answered/skipped resolution status, safe room discovery and dedicated player-token-authenticated join/start/skip/continue RPCs. It replaces the foundation launch guard with full definition validation, extends the generic answer RPC with untimed binary assigned scoring, auto-reveals under a session row lock, extends reconnect and player-safe state, and keeps Standard phase/scoring behaviour intact. Both migrations remain unapplied locally; no Supabase or Netlify release was performed. Quiz import/export v1 adds no migration after `007`; production remains at `005` and there is no migration `008`.
