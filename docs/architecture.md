...
Per-quiz themes use a central six-entry typed registry (`katwed`, `midnight`, `sunset`, `arcade`, `mint`, `paper`) and scoped CSS custom properties rather than separate components or a styling dependency. Katwed is the defensive/default value. `themeId` is part of `Quiz` and `QuizSaveInput`, and the safe game-state boundary carries only that curated ID to presentation and joined-player surfaces. `PresentationStage` therefore applies the same theme to the full presentation and compact controller preview, while controller/editor chrome and other host/admin routes remain unthemed. Covers remain independent library metadata, and the duplicate helper copies the theme while continuing to remap all quiz identities. This implementation is tested locally; migration `202608070004_quiz_themes.sql` and the matching frontend are pending a deliberate production release.
...
`202608070004_quiz_themes.sql` is the single pending migration. It adds the constrained, non-null `quizzes.theme_id` column with the backward-compatible `katwed` default and recreates `quiz_to_json`, `host_save_quiz` and `get_player_game_state` only as needed to carry the curated ID. A missing theme on an old-client insert defaults to Katwed; a completely absent theme key on an old-client update preserves the stored theme. The migration is statically tested but has not been applied, so the live production baseline remains `202608070003_storage_manager.sql`.
...
- broader background and visual-identity work beyond the six curated per-quiz themes;
