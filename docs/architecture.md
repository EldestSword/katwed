...
Shared game audio follows a separate presentation-only boundary. A pure mapper converts authoritative safe game state, the existing Double Score window and the current countdown into music/effect intent. One central `GameAudioEngine` owns two crossfading music elements and one sting element only inside `/present`; the Controller's compact preview and every Player route create none. Quiz `soundPackId` selects the registry entry, while mute and category volumes stay in local host-device preferences synchronised across Controller and Presentation windows. Stable event keys prevent one-shot replay during duplicate Realtime refreshes, Strict Mode and reconnect. Presentation-visible YouTube questions silence the background bed because the current iframe has no reliable playback-state events. See `docs/audio-language.md`.
