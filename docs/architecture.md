...
The live Supabase project has applied every migration through `202608070002_quiz_covers.sql`. Applied files are immutable history; production changes require a new chronological forward migration.
...
`202608070003_storage_manager.sql` is a pending forward migration. It adds owner-scoped authenticated listing for the existing `question-images` bucket and the bounded `host_classify_media_paths` RPC. The migration and matching frontend are implemented and tested locally but have not been applied or deployed. Production remains at `202608070002_quiz_covers.sql`.
