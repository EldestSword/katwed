...
The dashboard retains the parsed definition only for an explicit confirmation step and renders a separate spoiler-safe metadata summary; answer-bearing question data is never placed in the preview DOM. Export is read-only and available for Active and Archived quizzes. Version 1 preserves cover, question and option media references without uploading or embedding binary data, version 2 adds Typed Answer, and version 3 adds Standard scoring flags and tile-grid metadata. The v4 adapter adds the quiz-wide answer palette ID and exact eight-colour custom tuple. Exports target v4; imports retain v1-v3 compatibility, default old scoring flags as documented and normalise a missing palette to Classic. The transfer adapter changes no repository contract.
...
The live Supabase project has applied every committed migration through `202608260001_quiz_answer_palettes.sql`. Applied files are immutable history; production changes require a new chronological forward migration. The palette migration was applied after a remote history check and dry run identified it as the sole pending file. No Netlify deployment was performed.
