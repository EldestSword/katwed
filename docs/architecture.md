...
The dashboard retains the parsed definition only for an explicit confirmation step and renders a separate spoiler-safe metadata summary; answer-bearing question data is never placed in the preview DOM. Export is read-only and available for Active and Archived quizzes. Version 1 preserves cover, question and option media references without uploading or embedding binary data, and version 2 adds Typed Answer. The v3 adapter adds Standard scoring flags and tile-grid metadata; exports target v3 while imports retain v1/v2 compatibility and default their scoring flags to false. The transfer adapter changes no repository contract.
...
The production schema and matching Netlify frontend are deployed through `202608090001_fix_typed_answer_validation_trigger.sql`. The single pending migration is `202608090002_standard_scoring_and_tile_options.sql`; it is unapplied and the matching frontend is not deployed. Active and archived libraries are separate repository queries. Archive and permanent deletion reject quizzes with active rooms, archived quizzes cannot launch, and permanent deletion requires an archived quiz. A database trigger preserves those rules even if an authenticated client attempts a direct row operation.
...
The live Supabase project has applied every migration through `202608090001_fix_typed_answer_validation_trigger.sql`. Applied files are immutable history; production changes require a new chronological forward migration. `202608090002_standard_scoring_and_tile_options.sql` is the single pending migration. A deliberate release must apply it before deploying the matching Standard-scoring, Double Score, tile-grid and format-v3 frontend bundle.
...
`202608090002_standard_scoring_and_tile_options.sql` is pending and unapplied. It adds false-defaulted question columns, extends owner/safe serialisation and save, updates authoritative Standard answer scoring and question opening, and validates optional tile-grid metadata without invalidating legacy media. No Supabase migration or Netlify deployment was performed for this package.
