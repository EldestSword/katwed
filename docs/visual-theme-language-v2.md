...
The registry contains 12 choices including the platform UI stack and 11 redistributed Latin WOFF2 faces. Bricolage Grotesque remains the default display face and System UI the default utility face for the original six themes, preserving their current typography. All three reviewed batches use only approved role combinations. Each `@font-face` uses `font-display: swap`. The CSS lists all approved faces, but browsers fetch a font file only when rendered text uses that family. Catalogue cards deliberately use the backstage UI face; the selected themed preview and audience content request their actual theme fonts. No decorative font is preloaded and there are no third-party runtime font requests.
...
The reusable theme browser uses real buttons with `aria-pressed`, a labelled search field and simple category filter buttons. Search is case-insensitive and whitespace-tolerant across name, description, category label and keywords. The responsive grid retains explicit Selected/Choose text and keyboard focus. The 51-theme catalogue currently represents all nine registered categories, including Wildcards; unrepresented future categories do not create empty controls. Tests cover the real catalogue and a 48-item non-production fixture.

The original cards retain lightweight token swatches. Each imported card lazy-loads one reviewed 480×270 quality-68 preview WebP. The 45 Batch 1, Batch 2 and Batch 3 previews total 381,978 bytes, so opening Quiz Settings never requests the 135 imported full-resolution backgrounds. Background cards remain a separate step and lazy-load only the three images registered to the selected theme.
...
## Batch 2 measured performance

The 45 source PNGs total 165,457,097 bytes. Their production WebPs total 5,717,982 bytes, a 96.54% reduction and 127,066 bytes on average. `1960s-pop` is the smallest at 28,268 bytes; `punk-xerox` is the largest at 291,810 bytes. The 15 preview thumbnails add 126,896 bytes in total. Details are recorded in [`visual-theme-batch-2-size-report.json`](visual-theme-batch-2-size-report.json).
...
`theme-source/` is ignored by Git. `npm run import:theme-batch -- --batch <batch-01|batch-02|batch-03>` performs a no-write validation and measured conversion; add `--write` only for a reviewed package. A batch configuration supplies the reviewed source/content digests, exact contract, prior registered IDs and output names. Explicit `--source`, `--source-archive`, `--expected-content-sha256` and `--expected-archive-sha256` values may override those paths and digests for verification. Digest mismatches stop before output preparation or replacement, including with `--allow-existing`, and reports distinguish `sourceContentSha256` from `sourceArchiveSha256`. The importer uses the 2020-12 schema, exact reviewed batch contract, approved font roles, safe string checks and full Sharp decoding before it writes production assets, previews, trusted generated metadata, portable enums and a deterministic size report. Only the latest registered batch rewrites the shared portable schemas, so reproducing an earlier batch cannot remove later controlled IDs. Source masters are not committed.
...
3. Add one reviewed batch configuration, then run the reusable ingestion pass to validate contrast, optimise images and register stable IDs.
...
Batches 1, 2 and 3 add controlled IDs within portable format v5 semantics, so there is no format bump. Forward migration `202608300001_visual_theme_batch_1.sql` is required before production can persist Batch 1 IDs; `202608300002_visual_theme_batch_2.sql` depends on it and adds Batch 2; `202608300003_visual_theme_batch_3.sql` depends on Batch 2 and adds Batch 3. All three remain unapplied pending deliberate release approval.
