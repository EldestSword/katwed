...
The registry contains 12 choices including the platform UI stack and 11 redistributed Latin WOFF2 faces. Bricolage Grotesque remains the default display face and System UI the default utility face for the original six themes, preserving their current typography. Both reviewed batches use only approved role combinations. Each `@font-face` uses `font-display: swap`. The CSS lists all approved faces, but browsers fetch a font file only when rendered text uses that family. Catalogue cards deliberately use the backstage UI face; the selected themed preview and audience content request their actual theme fonts. No decorative font is preloaded and there are no third-party runtime font requests.
...
The reusable theme browser uses real buttons with `aria-pressed`, a labelled search field and simple category filter buttons. Search is case-insensitive and whitespace-tolerant across name, description, category label and keywords. The responsive grid retains explicit Selected/Choose text and keyboard focus. The 36-theme catalogue currently represents Katwed Originals, Abstract, Music, Decades, Cinematic, Places & Culture, Seasonal and Entertainment; unrepresented future categories do not create empty controls. Tests cover the real catalogue and a 48-item non-production fixture.

The original cards retain lightweight token swatches. Each imported card lazy-loads one reviewed 480×270 quality-68 preview WebP. The 30 Batch 1 and Batch 2 previews total 256,416 bytes, so opening Quiz Settings never requests the 90 imported full-resolution backgrounds. Background cards remain a separate step and lazy-load only the three images registered to the selected theme.
...
A built-in background belongs to exactly one theme. `backgroundsForTheme()`, runtime normalisation, save validation, portable enum handling and the pending production database constraint share the exact 36-by-three compatibility matrix. Theme default is still `null` and has no static image. A theme change clears an incompatible background; it never silently chooses or remembers a replacement.

Source artwork is local-only. The production target is a metadata-stripped quality-82 WebP at no more than 1920×1080, never upscaled and preserving 16:9. Batch 1's measurements are recorded in [`visual-theme-batch-1-size-report.json`](visual-theme-batch-1-size-report.json), and Batch 2's in [`visual-theme-batch-2-size-report.json`](visual-theme-batch-2-size-report.json).
