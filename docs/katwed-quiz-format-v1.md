# Katwed quiz portable format v1

Katwed quiz files are UTF-8 JSON documents with the filename suffix `.katwed.json`. Version 1 supports Standard and Head-to-Head quizzes and all six current question types: single choice, multiple select, true or false, slider, pinpoint and mash-up.

Exported files contain correct answers. Keep a file closed if the person importing it intends to play the quiz blind.

The machine-readable companion is [`schemas/katwed-quiz-v1.schema.json`](schemas/katwed-quiz-v1.schema.json). Katwed's runtime importer remains authoritative because it also validates cross-references, theme/background compatibility and the normal quiz-save rules.
