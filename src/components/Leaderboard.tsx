...
export function LeaderboardRow({ entry, currentPlayerId, visual, movement, emphasised, rowRef }: {
...
export function Leaderboard({
  ...
  return (
    <ol className={`leaderboard leaderboard--${variant}`} aria-label="Leaderboard" data-variant={variant}>
      {entries.map((entry) => <LeaderboardRow entry={entry} currentPlayerId={currentPlayerId} key={entry.playerId} />)}
    </ol>
  )
}
